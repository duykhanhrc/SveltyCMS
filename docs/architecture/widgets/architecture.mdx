---
path: "docs/architecture/widgets/architecture.mdx"
...
graph TB
    ...
    subgraph "Widget Store Management"
        WS[widget-store.svelte.ts<br/>State Management]
        WD[widgetDiscovery.ts<br/>Filesystem Scanner]
    end

    subgraph "Runtime System"
        CR[Collection Runtime<br/>Field Instances]
        MW[missing-widget.svelte<br/>Fallback Handler]
    end
...
## 🧩 Runtime Discovery & The 3-Layer System
...
sequenceDiagram
    ...
    User->>Store: Switch language (en → de)
...
- **Discovery**: The `widget-store` initializes with a `tenantId`, filtering the active list accordingly.
