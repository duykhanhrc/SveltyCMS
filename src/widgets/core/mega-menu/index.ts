...
export const validateMenuStructure = (
  ...
  const checkDepth = (item: MenuItem, currentDepth = 0): void => {
    if (currentDepth > maxDepth) {
      errors.push(
        `Menu item "${item._fields?.title || item._id}" exceeds maximum depth of ${maxDepth}`,
      );
    }
  ...
  const checkConstraints = (item: MenuItem): void => {
    ...
    if (maxChildren && childrenCount > maxChildren) {
      errors.push(
        `Menu item "${item._fields?.title || item._id}" has ${childrenCount} children, maximum allowed is ${maxChildren}`,
      );
    }
