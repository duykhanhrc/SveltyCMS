---
path: "src/widgets/core/rich-text/richtext.mdx"
...
<blockquote>
  **WYSIWYG editor** powered by Tiptap with configurable toolbar, multilingual support, and secure
  HTML storage for rich content editing.
</blockquote>
...
- ✅ **Configurable Toolbar**: Choose which editing features to enable (popovers for Links/Videos)
...
- ✅ **Table Support**: Visual table picker for creating complex layouts
- ✅ **Source View**: View and edit raw HTML code
- ✅ **Paste Options**: Easy paste as plain text feature
- ✅ **AI "Magic Wand" Translation**: One-click content localization via configured LLM
- ✅ **Global Accessibility (RTL)**: Automatic Right-to-Left detection and alignment for Middle Eastern locales

---

## AI Features

### Magic Wand Translation

The RichText widget features a dedicated **AI Translate** button in the toolbar (available when editing translatable content in a non-default language). This feature:

1. Extracts the HTML content from the default language version.
2. Utilizes the local Ollama or remote OpenAI provider to translate the content while **preserving HTML structure and formatting**.
3. Replaces the current editor content with the translated version instantly.
...
SveltyCMS provides first-class support for global audiences. The RichText widget automatically detects the active content language:

- **Automatic Alignment**: If the language is set to Arabic (`ar`), Hebrew (`he`), or Farsi (`fa`), the editor defaults to Right-to-Left (RTL) alignment.
- **Directional UI**: The underlying Tiptap engine applies `dir="rtl"` to the content editable area, ensuring that punctuation and cursor behavior follow RTL standards.
- **Toggleable Alignment**: Users can still manually override alignment using the standard alignment tools in the toolbar.
...
| `undo`      | Undo action           | Reverts last change             |
| `redo`      | Redo action           | Reapplies reversed change       |
| `source`    | Source Code View      | Toggle HTML view                |
| `paste`     | Paste Plain Text      | Inserts clipboard text          |
...
The RichText widget is part of SveltyCMS and is licensed under the Business Source License 1.1.
