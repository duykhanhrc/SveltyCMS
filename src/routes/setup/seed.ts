    ...
    const rolePromises = defaultRoles.map(async (role) => {
      try {
        ...
        if (result && "success" in result && !result.success) {
          ...
          logger.debug(
            `✅ Role "${role.name}" seeded successfully${tenantId ? ` for tenant ${tenantId}` : ""}`,
          );
        }
      } catch (error) {
        ...
        if (
          ...
          logger.error(
            `Failed to seed role "${role.name}"${tenantId ? ` for tenant ${tenantId}` : ""}:`,
            error,
          );
          throw error;
        }
...
export async function seedCollectionsForSetup(
  ...
  try {
    ...
    for (let i = 0; i < collections.length; i += BATCH_SIZE) {
        ...
        try {
          ...
          if (!firstCollection) {
            ...
            if (candidate?.path && candidate?.name) {
              ...
              logger.debug(
                `First collection identified: ${firstCollection.name} at ${firstCollection.path}`,
              );
            }
          }
          ...
          if (errorMessage.includes("already exists") || errorMessage.includes("duplicate")) {
            ...
            logger.error(
              `❌ Failed to create collection '${schema.name || "unknown"}'${tenantId ? ` for tenant ${tenantId}` : ""}: ${errorMessage}`,
            );
          }
        }
      });
      ...
      logger.debug(
        `📦 Seeded batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(collections.length / BATCH_SIZE)}`,
      );
    }
...
export async function seedSettings(
  ...
  try {
    ...
    throw new Error(
      `Cannot access database adapter: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
