...
function removeSystemLang(code: string) {
	...
	if (
		!systemSettings.systemLanguages.includes(
			systemSettings.defaultSystemLanguage,
		)
	) {
		systemSettings.defaultSystemLanguage =
			systemSettings.systemLanguages[0] || "en";
	}
}
...
function removeContentLang(code: string) {
	...
	if (
		!systemSettings.contentLanguages.includes(
			systemSettings.defaultContentLanguage,
		)
	) {
		systemSettings.defaultContentLanguage =
			systemSettings.contentLanguages[0] || "";
	}
}
...
$effect(() => {
	systemAvailable = availableLanguages.filter(
		(l: string) =>
			!systemSettings.systemLanguages.includes(l) &&
			l.toLowerCase().includes(systemPickerSearch.toLowerCase()),
	);
...
<form onsubmit={(e) => e.preventDefault()} class="fade-in w-full min-w-0">
  <div class="mb-6">
    <p
      class="text-sm text-center text-tertiary-500 dark:text-primary-500 sm:text-base"
    >
      {setup_system_intro()}
    </p>
  </div>
  ...
  <div class="space-y-2">
    {#if redisAvailable && !systemSettings.useRedis}
      <div
        class="rounded-lg bg-surface-500 p-4 text-white shadow-lg animate-in fade-in slide-in-from-top-4 duration-500"
        role="alert"
      >
        <div class="flex items-start gap-4">
          <iconify-icon
            icon="mdi:lightning-bolt"
            class="bg-error-500 p-2 text-white rounded-full text-2xl animate-pulse"
          ></iconify-icon>
          <div class="flex-1">
            <div class="flex items-center justify-between">
              <h4 class="font-bold text-lg flex items-center gap-4">
                ...
                <span
                  class="badge preset-filled-error-500 rounded-full uppercase"
                  >Recommended</span
                >
              </h4>
              <!-- Close button -->
            ...
            <p class="mt-1 text-sm leading-relaxed italic">
              A local Redis server was detected on this system. <br />Enabling
              Redis caching can speed up data access by up to
              <strong>50x</strong>
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-3 items-start">
        ...
        <div class="flex flex-col gap-1">
          ...
          <input
            ...
            type="text"
            placeholder={setup_system_site_name_placeholder?.() ||
              "My SveltyCMS Site"}
            class="input w-full rounded {displayErrors.siteName
              ? 'border-error-500'
              : 'border-slate-200'}"
            aria-invalid={!!displayErrors.siteName}
            aria-describedby={displayErrors.siteName
              ? "site-name-error"
              : undefined}
          />
        ...
        <div class="flex flex-col gap-1">
          ...
          >
            ...
            >
            <SystemTooltip
              title={setup_help_host_prod?.() || "The production URL..."}
            >
              <button
                type="button"
                tabindex="-1"
          ...
          <input
            ...
            placeholder={setup_system_host_prod_placeholder?.() ||
              "https://mysite.com"}
            class="input w-full rounded {displayErrors.hostProd
              ? 'border-error-500'
              : 'border-slate-200'}"
            aria-invalid={!!displayErrors.hostProd}
            aria-describedby={displayErrors.hostProd
              ? "host-prod-error"
              : undefined}
          />
        ...
        <div class="flex flex-col gap-1">
          ...
          >
            ...
            >
            <SystemTooltip
              title={setup_help_timezone?.() || "Default system timezone"}
            >
              <button
                type="button"
                tabindex="-1"
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2 items-start pt-1">
        ...
        <div class="space-y-2">
          ...
          >
            ...
            >
            <SystemTooltip
              title={setup_help_media_type?.() || setup_help_media_path()}
            >
              <button
                type="button"
                tabindex="-1"
          ...
          >
            <option value="local"
              >{setup_media_type_local?.() || "📁 Local Storage"}</option
            >
            <option value="s3"
              >{setup_media_type_s3?.() || "☁️ Amazon S3"}</option
            >
            <option value="r2"
              >{setup_media_type_r2?.() || "☁️ Cloudflare R2"}</option
            >
            <option value="cloudinary"
              >{setup_media_type_cloudinary?.() || "☁️ Cloudinary"}</option
            >
          </select>
        </div>
        ...
        <div class="space-y-2">
          ...
          >
            ...
            <SystemTooltip
              title={setup_help_media_type?.() || "Storage path configuration"}
            >
              <button
                type="button"
                tabindex="-1"
          ...
          {#if systemSettings.mediaStorageType !== "local"}
            <div
              class="rounded-md border border-amber-300/50 bg-amber-50/50 p-3 dark:border-amber-700/50 dark:bg-amber-900/20"
              role="status"
            >
              <p
    ...
    <!-- Languages -->
    <section
      class="space-y-5 border-t border-surface-200 dark:border-white/10 pt-2"
    >
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        <!-- Default System Language -->
        <div
          class="space-y-2 rounded border border-surface-200 dark:border-white/5 p-4"
        >
          <label
            for="default-system-lang"
            class="mb-1 flex items-center gap-1 text-sm font-medium"
          >
            <iconify-icon
              icon="mdi:translate"
          ...
          <p
            class="text-[10px] text-slate-500 dark:text-white/40"
            id="system-lang-help"
          >
          ...
          <div>
            <div
              class="mb-1 flex items-center gap-1 text-sm font-medium tracking-wide"
            >
              <iconify-icon
            ...
            <div
              class="relative flex min-h-10.5 flex-wrap items-center gap-2 rounded border border-surface-200 dark:border-white/5 p-2 pr-16"
            >
              {#each systemSettings.systemLanguages as lang (lang)}
                <span
              ...
              {#if systemAvailable.length}
                ...
                >
                  <iconify-icon icon="mdi:plus" width="14" aria-hidden="true"
                  ></iconify-icon>
                  {button_add?.() || "Add"}
                </button>
              {/if}
                ...
                >
                  ...
                  <div class="max-h-48 overflow-auto">
                    {#if systemAvailable.length === 0}
                      <p
                        class="px-1 py-2 text-center text-[11px] text-slate-400 dark:text-white/40"
                      >
        ...
        >
          ...
          <select
            ...
            class="input w-full rounded {displayErrors.defaultContentLanguage
              ? 'border-error-500'
              : ''}"
            aria-invalid={!!displayErrors.defaultContentLanguage}
            aria-describedby={displayErrors.defaultContentLanguage
              ? "default-content-lang-error"
              : undefined}
          >
          ...
          {#if displayErrors.defaultContentLanguage}
            <div
              id="default-content-lang-error"
              class="mt-1 text-xs text-error-500"
              role="alert"
            >
              {displayErrors.defaultContentLanguage}
            </div>
          {/if}
          <div>
            <div
                ...
                >
                  ...
                  <div class="max-h-48 overflow-auto">
                    {#if contentAvailable.length === 0}
                      <p
                        class="px-1 py-2 text-center text-[11px] text-white/40"
                      >
                        {setup_help_no_matches?.() || "No matches"}
                      </p>
                    {/if}
    ...
    <section
      id="redis-section"
      class="space-y-2 border-t border-surface-200 dark:border-white/10 pt-2"
    >
      <div class="rounded border border-surface-200 dark:border-white/5 p-3">
        <div class="flex items-center gap-2">
          ...
          <div class="flex items-center gap-2">
            ...
            <label
              for="use-redis"
              class="font-medium text-black dark:text-white text-sm"
            >
        ...
        {#if systemSettings.useRedis}
          <div
            class="grid grid-cols-1 gap-4 sm:grid-cols-3 animate-in fade-in slide-in-from-top-2 duration-300"
          >
            <div class="space-y-1.5 text-black dark:text-white">
              <label
                for="redis-host"
                class="text-xs font-semibold text-slate-500 dark:text-white/40"
                >Redis Host</label
              >
              <input
                id="redis-host"
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2 pb-4">
      <!-- Multi-Tenant Toggle -->
      <div
        class="input flex items-center gap-3 rounded border border-surface-200 dark:border-white/5 p-3"
      >
        <input
        ...
        <div class="flex items-center gap-2">
          ...
          <SystemTooltip
            title={setup_system_multi_tenant_desc?.() ||
